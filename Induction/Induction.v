...
Proof.
  intros n. destruct n as [| n1].
  - reflexivity.
  - induction n1 as [| n2 IH2].
    + reflexivity.
    + rewrite -> IH2. rewrite -> negb_involutive. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  assert (H3: (m + n) + p = m + (n + p)). { rewrite -> plus_assoc'. reflexivity. }
...
Proof.
  ...
  - reflexivity.
  - simpl. rewrite -> plus_swap. rewrite <- Hn1. reflexivity.
Qed.
...
Proof.
  ...
  induction m as [| m1 Hm1].
  { rewrite -> H1. reflexivity. }
  { rewrite -> mult_comm_1. rewrite <- Hm1. reflexivity. }
Qed.
...
Proof.
  ...
  induction n as [| n1 Hn1].
  { reflexivity. }
  { simpl. rewrite -> Hn1. rewrite -> plus_assoc'. reflexivity. }
Qed.
...
Proof.
  ...
  induction n as [| n1 Hn1].
  - reflexivity.
  - simpl. rewrite -> mult_plus_distr_r. rewrite -> Hn1. reflexivity.
Qed.
...
Theorem bin_to_nat_incr_new : forall x : bin,
  S (bin_to_nat(x)) = bin_to_nat(incr(x)).
Proof.
  induction x as [| x1 Hx1| x2 Hx2].
  { reflexivity. }
  { reflexivity. }
  { simpl. rewrite <- Hx2. simpl.
    replace (bin_to_nat x2 + S (bin_to_nat x2)) with
      (S(bin_to_nat x2) + bin_to_nat x2).
    reflexivity.
    { rewrite -> plus_comm. reflexivity. } }
Qed.
(** [] *)
...
Definition bin_div_2 (b : bin) : bin :=
  match b with
  | B => B
  | D b1 => b1
  | N b2 => b2
...
Definition bin_plus_2 (b : bin) : bin :=
  match  b with
  | B => D (N B)
  | D b1 => D (incr b1)
  | N b2 => N (incr b2)
...
Example test_bin_plus_2:    bin_plus_2 (D (D (N B))) = D (N (N B)).
Proof. simpl. reflexivity.  Qed.

Theorem bin_plus_2_e_ii : forall b: bin,
  bin_plus_2 b = incr (incr b).
Proof.
  induction b as [| b1 Hb1| b2 Hb2].
  { reflexivity. }
  { reflexivity. }
  { reflexivity. }
Qed.
...
Fixpoint nat_to_bin (n : nat) : bin :=
  ...
  | O => B
  | S O => N B
  | S (S n') => bin_plus_2(nat_to_bin(n'))
  end.

Theorem nat_to_bin_plus_1_e : forall n: nat,
  nat_to_bin(S n) = incr(nat_to_bin n).
Proof.
  destruct n as [| n1].
  { reflexivity. }
  { induction n1 as [| n2 Hn2].
    { reflexivity. }
    { rewrite -> Hn2. rewrite <- bin_plus_2_e_ii. reflexivity. } }
Qed.

Theorem nat_to_bin_e : forall n: nat,
...
Proof.
  ...
  { induction n1 as [| n2 Hn2].
    { reflexivity. }
    { rewrite -> nat_to_bin_plus_1_e.
      rewrite <- bin_to_nat_incr_new.
      rewrite -> Hn2.
      reflexivity. } }
Qed.
...
Example bin_to_nat_fail:    nat_to_bin(bin_to_nat(D (D B))) = B.
Proof. simpl. reflexivity.  Qed.
...
Fixpoint normalize (b : bin) : bin :=
  ...
  | B => B
  | N b1 => N (normalize b1)
  | D b2 => Doule_bin(normalize(b2))
  end.

Theorem Doule_bin_base : forall b: bin,
  Doule_bin(incr b) = D (incr b).
Proof.
  induction b as [| b1 Hb1| b2 Hb2].
  { reflexivity. }
  { reflexivity. }
  { simpl. rewrite -> Hb2. reflexivity. }
Qed.

Theorem plus_1_Doule_bin : forall b: bin,
  Doule_bin(incr (incr b)) = bin_plus_2(Doule_bin (incr b)).
Proof.
  induction b as [| b1 Hb1| b2 Hb2].
  { reflexivity. }
  { simpl. rewrite -> Doule_bin_base. reflexivity. }
  { simpl. rewrite -> Doule_bin_base. reflexivity. }
Qed.

Theorem nat_to_bin_D : forall n: nat,
  nat_to_bin(n + n) = Doule_bin(nat_to_bin(n)).
Proof.
  induction n as [| n1 Hn1].
  { reflexivity. }
  { rewrite -> nat_to_bin_plus_1_e.
    ...
    { reflexivity. }
    { rewrite -> nat_to_bin_plus_1_e.
      rewrite -> plus_1_Doule_bin.
      rewrite <- nat_to_bin_plus_1_e.
      rewrite <- Hn1.
      reflexivity. } }
Qed.

Theorem nat_to_bin_N : forall n: nat,
  nat_to_bin(S(n + n)) = N(nat_to_bin(n)).
Proof.
  induction n as [| n1 Hn1].
  { reflexivity. }
  { rewrite <- plus_n_Sm.
    assert (H: nat_to_bin (S (S (S n1 + n1))) = bin_plus_2(nat_to_bin (S (n1 + n1)))).
    { reflexivity. }
    rewrite -> H.
    rewrite -> Hn1.
    rewrite -> nat_to_bin_plus_1_e.
...
Theorem bin_to_nat_e : forall b: bin,
  nat_to_bin(bin_to_nat(b)) = normalize(b).
Proof.
  induction b as [| b1 Hb1| b2 Hb2].
  { reflexivity. }
  { simpl.
    rewrite -> nat_to_bin_D.
    rewrite -> Hb1.
    reflexivity. }
  { assert (H: bin_to_nat (N b2) = S((bin_to_nat b2) + (bin_to_nat b2)) ). { reflexivity. }
    rewrite -> H.
    rewrite -> nat_to_bin_N.
    rewrite -> Hb2.
    reflexivity. }
Qed.
